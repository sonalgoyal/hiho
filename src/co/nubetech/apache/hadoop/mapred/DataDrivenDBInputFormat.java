...
public class DataDrivenDBInputFormat<T extends DBWritable> extends
	...
	public static class DataDrivenDBInputSplit extends
		org.apache.hadoop.mapreduce.lib.db.DataDrivenDBInputFormat.DataDrivenDBInputSplit implements org.apache.hadoop.mapred.InputSplit {

		private String lowerBoundClause;
		private String upperBoundClause;
		org.apache.hadoop.mapreduce.lib.db.DataDrivenDBInputFormat.DataDrivenDBInputSplit split;
		...
		public DataDrivenDBInputSplit(org.apache.hadoop.mapreduce.lib.db.DataDrivenDBInputFormat.DataDrivenDBInputSplit split) {
			this.split = split;
			this.lowerBoundClause = split.getLowerClause();
			this.upperBoundClause = split.getUpperClause();
		}
	...
	public org.apache.hadoop.mapred.InputSplit[] getSplits(JobConf job, int chunks) throws IOException {
	    List<org.apache.hadoop.mapreduce.InputSplit> newSplits = 
	      this.getSplits(job);
	    org.apache.hadoop.mapred.InputSplit[] ret = new org.apache.hadoop.mapred.InputSplit[newSplits.size()];
	    int i = 0;
	    for (org.apache.hadoop.mapreduce.InputSplit s : newSplits) {
	      org.apache.hadoop.mapreduce.lib.db.DataDrivenDBInputFormat.DataDrivenDBInputSplit split = 
	    	(org.apache.hadoop.mapreduce.lib.db.DataDrivenDBInputFormat.DataDrivenDBInputSplit)s;
	      ret[i++] = new DataDrivenDBInputSplit(split);
	    }
	    return ret;
	  }
	...
	public List<InputSplit> getSplits(Configuration job) throws IOException {
		...
		int targetNumTasks = job.getInt(
				MRJobConfig.NUM_MAPS, 1);
		if (1 == targetNumTasks) {
			...
			singletonSplit.add(new org.apache.hadoop.mapreduce.lib.db.DataDrivenDBInputFormat.DataDrivenDBInputSplit("1=1", "1=1"));
		...
		try {
			...
			//		.getInputOrderBy()));
			return splitter.split(job, results, getDBConf()
							.getInputOrderBy());
	...
	protected RecordReader<LongWritable, T> createDBRecordReader(
			org.apache.hadoop.mapreduce.lib.db.DBInputFormat.DBInputSplit split, Configuration conf) throws IOException {
	...
	private List<InputSplit> convertSplit(List<InputSplit> splits) {
		List<InputSplit> returnSplits = new ArrayList<InputSplit>();
		for (InputSplit split: splits) {
			org.apache.hadoop.mapreduce.lib.db.DataDrivenDBInputFormat.DataDrivenDBInputSplit dbSplit = 
				(org.apache.hadoop.mapreduce.lib.db.DataDrivenDBInputFormat.DataDrivenDBInputSplit) split;
			DataDrivenDBInputSplit returnSplit = new DataDrivenDBInputSplit(dbSplit);
			returnSplits.add(returnSplit);
		}
		return returnSplits;
	}
}
